...
public class ChangeCamera : MonoBehaviour {
    ...
    void Update()
    {
        ...
        if (Input.GetKey(KeyCode.X)||Input.GetKeyDown(KeyCode.X))
        {
            if(cameras[currentCameraIndex].tag == "MainCamera")
            {
                cameras[currentCameraIndex].transform.LookAt(centroid.transform);
                //fpsController.transform.LookAt(centroid.transform);
            }
        }
    }
