...
public class SmoothFollow : MonoBehaviour
{
    ...
    //basically a lerp version of Vector3.LookAt
    Quaternion SmoothLook()
    {
        Vector3 lookVector = (target.position - transform.position).normalized;
        Quaternion targetRotation = Quaternion.LookRotation(lookVector);
        return Quaternion.Lerp(transform.rotation, targetRotation, Time.deltaTime * rotationDamping);
    }
