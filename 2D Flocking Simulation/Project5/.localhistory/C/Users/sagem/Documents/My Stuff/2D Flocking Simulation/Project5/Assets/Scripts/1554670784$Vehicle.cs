...
public abstract class Vehicle : MonoBehaviour
{
    ...
    protected void Update()
    {
        ...
        {
            //collisionManager.humans.Remove(gameObject);
            //Destroy(futurePosObject);
            Destroy(gameObject);
        }
        ...
        //fix any unwanted changes to the y axis
    ...
    /// but if it did this wouldnot work
    /// </summary>
    ...
    {        
        //PROBABLY wrong direction for the vector
        if (vehiclePosition.x + boundsDistanceBuffer > worldBounds.x)
        {
            ...
            return Flee(new Vector3(worldBounds.x, 0, vehiclePosition.z)) * stayInBoundsWeight;
        }
        else if (vehiclePosition.x - boundsDistanceBuffer < 0)
        {
            //Debug.Log("neg x bound");
            return Flee(new Vector3(0, 0, vehiclePosition.z)) * stayInBoundsWeight;

        }
        else if (vehiclePosition.z + boundsDistanceBuffer > worldBounds.z)
        {
            //Debug.Log("pos y bound");
            return Flee(new Vector3(vehiclePosition.x, 0, worldBounds.z)) * stayInBoundsWeight;

        }
        else if (vehiclePosition.z - boundsDistanceBuffer < 0)
        {
            //Debug.Log("neg y bound");
            return Flee(new Vector3(vehiclePosition.x, 0, 0)) * stayInBoundsWeight;
        }
    ...
    #region steering forces (seek, flee, etc)
    // SEEK METHOD
    ...
    /// <summary>
    /// Makes the vehicle wander randomly
    /// </summary>
    public Vector3 Wander()
    {
        ...
        {
            wanderAngle += Random.Range(-wanderAngleVariance, wanderAngleVariance); //change the wander angle by a random amount within a range
        ...
        wanderToPoint = wanderCircleLocation + ((Quaternion.AngleAxis(wanderAngle, Vector3.up) * transform.right) * wanderCircleRadius);
